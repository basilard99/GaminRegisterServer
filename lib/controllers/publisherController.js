'use strict';

var publisherFactory = require('../models/publisher.js');
var Promise = require('bluebird');

var UNKNOWN_ERROR = 'BI: An unknown error occurred';

var PublisherControllerObj = function definePublisherController(dataService) {
    this._dataService = dataService;
}

// TODO: No reason to pass in request. Just pass in body
PublisherControllerObj.prototype.put = function putAction(req) {
    var _this = this;

    return new Promise(function putPublisherPromise(resolve, reject) {
        var pub;

        try {
            pub = publisherFactory.createPublisher(req.body.name,
                                                   req.body.webSite,
                                                   req.body.code,
                                                   req.body.isActive,
                                                   req.body.description
            );
        } catch (e) {
            e.httpStatus = 400;
            reject(e);
        }

        _this._dataService.savePublisher(pub)
            .then(function savePublisherSuccessful(model) {
                resolve({ httpStatus: 201, data: model });
            }).catch(function savePublisherFailed(e) {
                e.httpStatus = 500;
                reject(e);
            });
    });
};

// TODO: No reason to pass in request. Just pass in publishername
PublisherControllerObj.prototype.get = function getAction(req) {
    var _this = this;

    return new Promise(function getPublisherPromise(resolve, reject) {
        _this._dataService.getPublisher(req.params.publisherName)
            .then(function getPublisherSuccessful(result) {
                if (!result) {
                    e.httpStatus = 404;
                    reject(e);
                } else {
                    resolve({ httpStatus: 200, data: result });
                }
            }).catch(function getPublisherFailed(e) {
                e.httpStatus = 500;
                reject(e);
            });
    });
};

PublisherControllerObj.prototype.patch = function patchAction(req) {
    var _this = this;

    return new Promise(function patchPublisherPromise(resolve, reject) {
        var pub;

        try {
            pub = publisherFactory.createPublisher(req.body.name,
                                                   req.body.webSite,
                                                   req.body.code,
                                                   req.body.isActive,
                                                   req.body.description);
        } catch (e) {
            e.httpStatus = 400;
            reject(e);
        };

        _this._dataService.savePublisher(pub, function callback(err, model) {
            if (err) {
                err.httpStatus = 500;
                reject(err);
            }

            resolve({ httpStatus: 201, data: model });
        });
    });
};

module.exports.createPublisherController = function createPublisherController(dataService) {
    return new PublisherControllerObj(dataService);
};
