	...
	return {
		savePublisher: function(publisher, callback) {
			...
			pub.where({ name: publisher.name }, function(err, results) {
				...
				if (model)
				{					
					if (publisher.webSite) model.webSite = publisher.webSite;
					if (publisher.code) model.code = publisher.code;
					if (publisher.isActive) model.isActive = publisher.isActive;
					if (publisher.description) model.description = publisher.description;
